---
title: "CLSA PA and Sleep Analysis and Output: REVISED August 5, 20222"
author: "RSF"
date: "08/04/2022"
...
8) Reviewer requested information - Differences in physical activity, sleep duration, and cognition based on age group and biological sex (requested by Reviewer 2)

```{r}
compfinal2$AgeGroup<-NA
compfinal2$AgeGroup[compfinal2$Age<=54]<-"45-54 years"
compfinal2$AgeGroup[compfinal2$Age>54 & compfinal$Age<=64]<-"55-64 years"
compfinal2$AgeGroup[compfinal2$Age>64 & compfinal$Age<=74]<-"65-74 years"
compfinal2$AgeGroup[compfinal2$Age>74 & compfinal$Age<=84]<-"75-84 years"
compfinal2$AgeGroup[compfinal2$Age>84]<-"85+ years"

Males<-subset(compfinal2, SEX_ASK_COM=="M")
Females<-subset(compfinal2, SEX_ASK_COM=="F")

library(psych)
#Males
describeBy(Males$Memory, Males$AgeGroup)
describeBy(Males$EF, Males$AgeGroup)
describeBy(Males$Verbal, Males$AgeGroup)
describeBy(Males$PASE_TOTAL, Males$AgeGroup)
describeBy(Males$SQ_Hours, Males$AgeGroup)

#Females
describeBy(Females$Memory, Females$AgeGroup)
describeBy(Females$EF, Females$AgeGroup)
describeBy(Females$Verbal, Females$AgeGroup)
describeBy(Females$PASE_TOTAL, Females$AgeGroup)
describeBy(Females$SQ_Hours, Females$AgeGroup)
```

9) Using FDR to determine adjusted p-values for each model of physical activity, sleep, and cognitive performance (Request from Reviewer 3) 

```{r}
#Model examining age and sex differences in PA (Age, Sex, Education, Income, ADLs, CESD, BMI, Age*Sex)
p.adjust(c(0.00001, 0.00001, 0.2958, 0.00001, 0.00001, 0.00001, 0.0137, 0.00001), "fdr")

#Model examining age and sex differences in total sleep time (Age, Sex, Education, Income, ADLs, CESD, BMI, Age*Sex)
p.adjust(c(0.00001, 0.00001, 0.0015, 0.0766, 0.0003, 0.00001, 0.0010, 0.00001), "fdr")

#Model examining if PA moderates the relationships between age and Memory, and whether these relationships are sex dependent 
#(Age, Sex, PASE Score, Age*Sex, Age*PASE Score, Sex*PASE Score, Age*Sex*PASE Score)
p.adjust(c(0.00001, 0.00001, 0.0044, 0.00001, 0.1216, 0.2194, 0.4328), "fdr")

#Model examining if PA moderates the relationships between age and Executive Function, and whether these relationships are sex dependent 
#(Age, Sex, PASE Score, Age*Sex, Age*PASE Score, Sex*PASE Score, Age*Sex*PASE Score)
p.adjust(c(0.00001, 0.00001, 0.00001, 0.00001, 0.2786, 0.7567, 0.6154), "fdr")

#Model examining if PA moderates the relationships between age and Verbal Fluency, and whether these relationships are sex dependent 
#(Age, Sex, PASE Score, Age*Sex, Age*PASE Score, Sex*PASE Score, Age*Sex*PASE Score)
p.adjust(c(0.00001, 0.00001, 0.00001, 0.0003, 0.6360, 0.9039, 0.7894), "fdr")

#Model examining if total sleep time moderates the relationships between age and Memory, and whether these relationships are sex dependent 
#(Age, Sex, PASE Score, Age*Sex, Age*PASE Score, Sex*PASE Score, Age*Sex*PASE Score)
p.adjust(c(0.00001, 0.00001, 0.0002, 0.00001, 0.0088, 0.2093, 0.0924), "fdr")

#Model examining if total sleep time moderates the relationships between age and Executive Function, and whether these relationships are sex dependent 
#(Age, Sex, PASE Score, Age*Sex, Age*PASE Score, Sex*PASE Score, Age*Sex*PASE Score)
p.adjust(c(0.00001, 0.00001, 0.00001, 0.00001, 0.0155, 0.0237, 0.0096), "fdr")

#Model examining if total sleep time moderates the relationships between age and Verbal Fluency, and whether these relationships are sex dependent 
#(Age, Sex, PASE Score, Age*Sex, Age*PASE Score, Sex*PASE Score, Age*Sex*PASE Score)
p.adjust(c(0.00001, 0.00001, 0.00001, 0.00001, 0.0003, 0.0297, 0.0074), "fdr")
```
